...
class TestFaceComparison:
    ...
    def test_existing_images(self):
        """Test comparison with existing captured images"""
        print("🧪 Testing with existing captured images...")
        
        if not os.path.exists(self.reference_face_path):
            print(f"❌ Reference image not found: {self.reference_face_path}")
            return False
        
        if not os.path.exists(self.detection_face_path):
            print(f"❌ Detection image not found: {self.detection_face_path}")
            print("💡 Capturing detection image...")
            if not self.capture_face(self.detection_face_path, "detection"):
                return False
        
        # Compare the images
        return self.compare_and_display()
...
def main():
    ...
    try:
        ...
        face_system = TestFaceComparison()
        
        print("🤖 Face Comparison System")
        print("Choose an option:")
        print("1. Test with existing images")
        print("2. Capture new images and compare")
        
        try:
            choice = input("Enter your choice (1 or 2): ").strip()
        except:
            choice = "1"  # Default to test mode
        
        if choice == "1":
            print("\n🧪 Running in test mode...")
            result = face_system.test_existing_images()
        elif choice == "2":
            print("\n📸 Running in capture mode...")
            result = face_system.run_full_comparison()
        else:
